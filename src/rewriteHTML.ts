...
import { createDataURI, injectDocumentJS, parseDataURI, routeURL } from './routeURL.js';
...
export function routeHTML(resource: StompURL, url: StompURL, config: Config) {
	if (resource.url.protocol === 'data:') {
		...
		return createDataURI({ mime, data: modifyHTML(data, url, config), attributes });
	}

	return routeURL('html', resource);
}

const essentialNodes = ['#documentType',
	'#document',
	'#text',
	'html',
	'head',
...
export function modifyHTML(
	...
	const tree: Element = fragment ? <Element>parseFragment(script) : <Element><unknown>parse(script);
		...
		},
		{
			...
			namespaceURI: 'http://www.w3.org/1999/xhtml',
				childNodes: [
					<TextNode>{
						nodeName: '#text',
						value: `if(typeof createClient!=='function')document.write("Stomp client failed to inject.");createClient(${JSON.stringify(config)}, ${JSON.stringify(url.codec.key)})`,
					},
				],
				attrs: [
					{
